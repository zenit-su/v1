  ...
  const handleSliderChange = useCallback(
    (newValue) => {
      ...
      if (newValue > sliderValue && newValue <= wordCount) {
        // Handle the first word revelation and playback
        if (
          currentIndex === 0 &&
          newValue === 0 &&
        ...
        // If newValue is less than wordCount (not beyond the last word)
        ...
        // If newValue equals wordCount (slider at extra step)
        if (newValue === wordCount) {
          // Check if there is an associated video
          if (slides[currentIndex]?.associatedVideo) {
            // Play associated video if not already playing
            if (!shouldPlayAssociatedVideo) {
              setShouldPlayAssociatedVideo(true);
            }
          } else {
            // No associated video, proceed to next slide
            handleNextSlide();
          }
        }
      }
  ...
  return (
    <>
      ...
      <ContentWrapper>
        ...
        <SliderComponent
          value={sliderValue}
          max={words.length} // Ensure this includes the extra step
